...
export function compactOperationalTitle(title: string, layerId: string) {
  const normalized = title.trim().toUpperCase();

  if (normalized.includes("NOTAM")) return "NOTAMS";
  if (normalized.includes("AERODROME")) return "AERODROMES";
  if (normalized.includes("FIRE")) return "ACTIVE FIRES";
  if (normalized.includes("EARTHQUAKE")) return "EARTHQUAKES";
  if (normalized.includes("WEATHER")) return "SEVERE WEATHER";
  if (layerId === "day_night") return "DAY/NIGHT";
...
export function compactOperationalMeta(meta: string, title: string) {
  const normalizedTitle = title.trim().toUpperCase();
  const normalizedMeta = meta.trim().toUpperCase();

  if (normalizedMeta.includes("NOT LOADED")) return "NOT LOADED";
  if (normalizedMeta.includes("ITEM")) return "";
...
export function compactLayerMetric(raw: string) {
...
export function compactHazardMeta(periodLabel?: string | null) {
  if (!periodLabel) return "";

  const normalized = periodLabel.trim().toUpperCase();
  if (normalized === "24H" || normalized === "5 MIN") return "";
  return normalized;
}
...
function LayerListRow({
  ...
  return <div className={styles.layerList}>{children}</div>;
}

export function MapStageLayerPanel({
  basemapMode,
  drawMode,
  drawPointsCount,
  layerRows,
  ...
  ].filter((section) => section.visible !== false) as Array<{
          ...
          <div className={styles.layerDetailGroup}>
            ...
            <div className={`${styles.layerDetailList} ${styles.layerDetailListStatic}`}>
              <div className={styles.basemapSwitchVertical}>
                {(["map", "satellite", "terrain3d"] as const).map((mode) => (
                  <button
                    key={mode}
                    className={`${styles.basemapButton} ${basemapMode === mode ? styles.basemapButtonActive : ""}`}
                    onClick={() => onSetBasemapMode(mode)}
                    type="button"
                  >
                    {mode === "terrain3d" ? "3D" : mode[0].toUpperCase() + mode.slice(1)}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className={styles.layerDetailGroup}>
            ...
            <div className={styles.drawSection}>
              <div className={styles.drawActions}>
                ...
                {drawMode ? (
                  <>
                    <button className={styles.basemapButton} disabled={drawPointsCount < 3} onClick={onFinishGeofence} type="button">
                      Finish
                    </button>
            ...
            <div className={styles.layerDetailList}>
              <LayerListRow>
                {naturalHazardRows.map((row) => (
                  <label key={row.id} className={styles.layerRow}>
                    <span className={styles.layerText}>
                      <strong>
                        <span aria-hidden="true" className={styles.layerSwatch} style={{ "--layer-color": row.color } as CSSProperties} />
                ...
                <label className={styles.layerRow}>
                  <span className={styles.layerText}>
                    <strong>
                      <span aria-hidden="true" className={styles.layerSwatch} style={{ "--layer-color": "#448aff" } as CSSProperties} />
                      <span aria-hidden="true" className={styles.layerGlyph} style={{ "--layer-color": "#448aff" } as CSSProperties}>D</span>
                      <span>DAY/NIGHT</span>
                    </strong>
                    <span>5 MIN</span>
                  </span>
  ...
  return (
    <section className={styles.layerSelector} id="layer-panel">
      ...
      <div className={styles.layerRailShell}>
        ...
        <div className={styles.layerDetailPanel}>{renderSectionDetail()}</div>
      </div>
    </section>
  );
}
