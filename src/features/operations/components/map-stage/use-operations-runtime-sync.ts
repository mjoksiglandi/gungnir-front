...
export function resolveRuntimeAssetIdentity(
  ...
  const missionAssignedCallsign = device
    ? missions
      .flatMap((mission) => mission.assignedDevices ?? [])
      .find((assignment) => assignment.deviceId === device.id)
      ?.callsign?.trim()
    : undefined;
  ...
  if (missionAssignedCallsign) {
    return {
      callsign: missionAssignedCallsign,
...
export function mergeAssetsWithRuntimeState(
  ...
  return assets.map((asset) => {
    ...
    const identity = resolveRuntimeAssetIdentity(asset, device, missions, track, latestTelemetry);
